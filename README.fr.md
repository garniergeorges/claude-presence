...
Les sessions sans heartbeat sont purgées après 24 heures. Tu peux forcer le nettoyage immédiat avec `claude-presence clear`, ou forcer la libération d'un verrou spécifique via l'outil MCP `resource_release` avec `force: true`.
...
Rétention :
- **Sessions** : purgées après 24 heures sans heartbeat.
